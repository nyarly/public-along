class AdpService
  ...
  def populate_workers
    ...
    begin
      ...
      str = get_json_str("https://uat-api.adp.com/hr/v2/workers?$select=workers/workerStatus&$top=1&count=true")
      json = JSON.parse(str)
    ...
    # while pos <= count
      begin
        ...
        str = get_json_str("https://uat-api.adp.com/hr/v2/workers?$top=50&$skip=#{pos}")
      end
      json = JSON.parse(str)
      # puts json: json
      json["workers"].each do |w|
        ...
        dept = work_assignment["homeOrganizationalUnits"].find { |ou| ou["typeCode"]["codeValue"] == "Department"}["nameCode"]
  ...
  def find_employee_type(json)
    json["shortName"] if json["workerTypeCode"]
  end
