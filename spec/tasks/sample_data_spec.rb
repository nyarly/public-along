...
describe 'db:sample_data', type: :tasks do
  before :all do
    ...
    Location.create(name: 'OT San Francisco', kind: 'Office')
    Rake.application = Rake::Application.new
    Rake.application.rake_require 'lib/tasks/sample_data', [Rails.root.to_s], ''
    Rake::Task.define_task :environment
  end

  describe 'db:sample_data:load' do
  ...
  describe 'db:sample_data:lebowski' do
    it 'should load lebowski' do
      Rake::Task['db:sample_data:lebowski'].invoke
      expect(Employee.count).to eq(1)
      expect(Employee.first.last_name).to eq('Lebowski')
      expect(Employee.first.profiles.active.reorder(:created_at).last.job_title.name).to eq('Test')
