...
RSpec.describe EmpDelta, type: :model do
  ...
  it 'meets validations' do
    ...
    expect(emp_delta).not_to allow_value(nil).for(:employee_id)
  end
  ...
  context 'report group' do
    ...
    it 'includes the correct data' do
      ...
      expect(EmpDelta.report_group).not_to include(non_report)
    end
  end

  context 'format attributes' do
    it 'onlies format select attributes' do
      ...
      expect(delta.format(delta.before)).not_to include('leave start date')
      ...
      expect(delta.format(delta.after)).not_to include('leave start date')
    end

    it 'subs names for ids' do
    ...
    it "subs nil if it can't find the activerecord object" do
  ...
  context 'format by value' do
    it 'collects the values and format them' do
    ...
    it 'is able to format both manager pk and manager employee id' do
      ...
      delta_a = FactoryGirl.create(:emp_delta,
        ...
        before: { 'manager_adp_employee_id' => manager_referenced_by_adp_emp_id_1.current_profile.adp_employee_id.to_s },
        after: { 'manager_adp_employee_id' => manager_referenced_by_adp_emp_id_2.current_profile.adp_employee_id.to_s })

      delta_b = FactoryGirl.create(:emp_delta,
        before: { 'manager_id' => manager_referenced_by_id_1.id.to_s },
        after: { 'manager_id' => manager_referenced_by_id_2.id.to_s })

      expect(delta_a.format_by_key).to eq([{ 'name' => 'Manager Employee ID', 'before' => manager_referenced_by_adp_emp_id_1.current_profile.adp_employee_id.to_s, 'after' => manager_referenced_by_adp_emp_id_2.current_profile.adp_employee_id.to_s }])
      expect(delta_b.format_by_key).to eq([{ 'name' => 'Manager', 'before' => manager_referenced_by_id_1.cn.to_s, 'after' => manager_referenced_by_id_2.cn.to_s }])
    end

    it 'excludes address information' do
      employee = FactoryGirl.create(:employee, :with_profile)

      delta = FactoryGirl.create(:emp_delta,
        employee: employee,
        before: { 'city' => 'city', 'line_1' => 'address', 'business_title' => 'biztitle' },
        after: { 'city' => 'city2', 'line_1' => 'address2', 'business_title' => 'biztitle2' })

      expect(delta.format_by_key).to eq([{ 'name' => 'City', 'before' => '', 'after' => '' }, { 'name' => 'Line 1', 'before' => '', 'after' => '' }, { 'name' => 'Business Title', 'before' => 'biztitle', 'after' => 'biztitle2' }])
    end
  end
  ...
  context 'build from profile' do
    ...
    it 'takes a dirty activerecord profile model and create an employee delta' do
